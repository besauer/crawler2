...
4. Unter „Einstellungen“ den OpenAI-API-Schlüssel sowie – falls die Funktion „Verwandte & semantische Keywords finden“ genutzt werden soll – den Google-Keyword-Planner-Schlüssel hinterlegen. Außerdem lassen sich dort alle Standardwerte verwalten: Synonym-Prompt, Modell, Temperatur und Vorschlagsanzahl, die Crawl-Vorgaben (maximale Seiten pro Start-URL, parallele Abrufe) sowie das Standard-Setup für den Keyword-Finder (Prompt, Modell, Temperatur, maximale Treffer). Alle Schlüssel werden ausschließlich im laufenden Prozess gespeichert, die Standardwerte landen in `settings.json`.
...
- Alle zentralen Daten (Einstellungen, Stammdaten, gespeicherte Suchen sowie die Synonym- und Keyword-Finder-Caches) werden im Verzeichnis `backups/` alle fünf Minuten automatisch gesichert. Beim Start des Servers werden fehlende Dateien aus `backups/auto_backup.json` wiederhergestellt.
