...
4. Unter „Einstellungen“ den OpenAI-API-Schlüssel sowie – falls die Funktion „Verwandte & semantische Keywords finden“ genutzt werden soll – den Google-Keyword-Planner-Schlüssel hinterlegen. Zusätzlich können in derselben Ansicht die Zugangsdaten für die Google Custom Search API sowie die Standardparameter für die Datenqualitätsprüfung gepflegt werden (Modell, Temperatur, Confidence-Schwelle, maximale Treffer, Batch-Größe, Suchsprache/-region sowie Positiv-/Negativlisten für Domains). Alle Schlüssel und Konfigurationen werden dauerhaft in `crawler_data.db` abgelegt und beim Start automatisch geladen. Im unteren Bereich steht außerdem die Datenverwaltung bereit, um komplette Sicherungen (inkl. API-Schlüssel) zu exportieren oder – nach vorheriger Vorschau – wieder einzuspielen.
5. Im Menüpunkt „Datenqualität“ lassen sich die in den Stammdaten hinterlegten URLs überprüfen. Die Seite zeigt Status, letzte Prüfergebnisse, KI-Begründungen und ggf. vorgeschlagene Alternativ-URLs. Die Prüfung läuft asynchron und bezieht sich ausschließlich auf die markierten Schulen. Über Schnellauswahl-Schaltflächen lassen sich 100, 200, 500 oder 1000 Datensätze mit dem Status „Noch nicht geprüft“ mit einem Klick vorauswählen; weitere Buttons aktivieren oder deaktivieren sämtliche Einträge. Während der Lauf aktiv ist, erscheinen Fortschrittsbalken, Meldungen und eine Live-Tabelle, die sämtliche aktuell gefundenen Empfehlungen samt Status, Sicherheit und Begründung auflistet. Vorschläge lassen sich anschließend mit einem Klick übernehmen, verwerfen oder die URL kann manuell korrigiert werden. Datensätze, bei denen ein Vorschlag übernommen wurde, erhalten automatisch zwei Status-Badges („OK“ plus „Richtiger Wert übernommen“). Alle Ergebnisse werden automatisch in `crawler_data.db` gespeichert und lassen sich weiterhin als JSON exportieren. Während der Prüfung wird dabei strikt folgender Ablauf eingehalten:
