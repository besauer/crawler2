...
6. Der neue Menüpunkt „Index“ dient dazu, gezielt Webseiten zu indizieren. Die Oberfläche ist in drei Bereiche gegliedert:
   - **Schulauswahl** – identisch zur Suchseite lassen sich Stammdaten filtern, automatisch vorausgewählte Schulen übernehmen und per Checkbox anpassen. Textfilter, boolesche Merkmale, numerische Vergleiche sowie die Buttons „Alle auswählen“, „Keine auswählen“, „Sichtbare auswählen“ und „Filter zurücksetzen“ stehen zur Verfügung. Unter der Tabelle wird jederzeit die Anzahl markierter Schulen angezeigt.
   - **Index-Einstellungen** – hier werden Crawltiefe, Parallelität, Rate-Limit, maximale Seiten pro Domain, Anzahl der Retry-Versuche, robots.txt-Respekt, PDF-Verarbeitung sowie Pfad-/Parameterlisten gepflegt. Die Werte sind bereits mit den Standardparametern aus den Einstellungen vorbelegt und können vor jedem Lauf angepasst werden.
   - **Laufstatus & Historie** – nach dem Start erscheint ein Fortschrittsbalken inklusive Statistiken (entdeckte Seiten, abgearbeitete Seiten, Dokumente, Links, Fehler) sowie die zuletzt analysierte URL. Alle Logmeldungen werden live eingeblendet. Unterhalb befindet sich eine Tabelle aller bisherigen Läufe samt Status, Dokument-/Linkanzahl und Fehlern. Ein Klick auf „Details“ öffnet eine Registerkarte mit Dokumenten (inkl. Filter), extrahierten Links und Rohlogs. Läufe können jederzeit abgebrochen werden; laufende Jobs setzen nach einem Seiten-Reload automatisch ihre Statusanzeige fort.
7. Im Menüpunkt „Suche anlegen“ besteht die Seite aus drei Abschnitten:
...
8. Im Menü „Suchen verwalten“ werden sämtliche Definitionen, Läufe und Ergebnisse verwaltet:
...
9. Unter „Debugging“ werden sämtliche Fehlerereignisse gesammelt. Die Ansicht zeigt Zeitpunkt, Kategorie, Fehlermeldung, strukturierte Kontextdaten (z. B. HTTP-Methode, Pfad, IP-Adresse) sowie den vollständigen Stacktrace. Über die Buttons können die Protokolle neu geladen oder als JSON exportiert werden; das Paket wird außerdem in vollständige Daten-Exporte integriert.
