...
4. Unter „Einstellungen“ den OpenAI-API-Schlüssel sowie – falls die Funktion „Verwandte & semantische Keywords finden“ genutzt werden soll – den Google-Keyword-Planner-Schlüssel hinterlegen. Zusätzlich können in derselben Ansicht die Zugangsdaten für die Google Custom Search API sowie die Standardparameter für die Datenqualitätsprüfung gepflegt werden (Modell, Temperatur, Confidence-Schwelle, maximale Treffer und Batch-Größe). Alle Schlüssel werden ausschließlich im laufenden Prozess gespeichert, die Standardwerte landen in `settings.json`. Im unteren Bereich steht außerdem die Datenverwaltung bereit, um komplette Sicherungen (inkl. API-Schlüssel) zu exportieren oder – nach vorheriger Vorschau – wieder einzuspielen.
5. Im Menüpunkt „Datenqualität“ lassen sich die in den Stammdaten hinterlegten URLs überprüfen. Die Seite zeigt Status, letzte Prüfergebnisse, KI-Begründungen und ggf. vorgeschlagene Alternativ-URLs. Über „Prüfung starten“ können alle oder ausgewählte Schulen asynchron bewertet werden; der Fortschritt erscheint als Live-Anzeige mit Fortschrittsbalken, Meldungen und Detail-Panel. Vorschläge lassen sich mit einem Klick übernehmen oder verwerfen, einzelne Datensätze können manuell korrigiert oder für eine erneute Prüfung vorgemerkt werden. Alle Ergebnisse werden in `data_quality_results.json` gespeichert und lassen sich als JSON exportieren.
...
- Ergebnisse der URL-Qualitätsprüfung werden in `data_quality_results.json` gesichert und können über „Datenqualität → Exportieren“ als JSON-Datei heruntergeladen werden.
...
- Alle zentralen Daten (Einstellungen, Stammdaten, gespeicherte Suchen, Datenqualitäts-Ergebnisse sowie die Synonym- und Keyword-Finder-Caches) werden im Verzeichnis `backups/` alle fünf Minuten automatisch gesichert. Beim Start des Servers werden fehlende Dateien aus `backups/auto_backup.json` wiederhergestellt.
